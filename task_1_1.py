...
При использовании рекурсии для стека вызовов происходит дополнительное выделение 1.8 MiB памяти.
Для того, чтобы дополнительная память не выделялась, я использовал цикл. Благодаря этому, дополнительное
выделение памяти не требуется, вычисления происходят для одной переменной sum_num.
"""
